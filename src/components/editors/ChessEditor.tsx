    ...
    return (
      <Stack gap="sm">
        ...
        <Grid gutter="sm">
          ...
          <Grid.Col span={4}>
            <NumberInput label="干员等级" value={chess.status.charLevel} min={1} onChange={v => patchChessStatus(chessId, { charLevel: Number(v) })} />
          </Grid.Col>
          <Grid.Col span={4}>
            <NumberInput label="技能等级" value={chess.status.skillLevel} min={1} max={10} onChange={v => patchChessStatus(chessId, { skillLevel: Number(v) })} />
          </Grid.Col>
          <Grid.Col span={4}>
            <NumberInput label="信赖值" value={chess.status.favorPoint} min={0} onChange={v => patchChessStatus(chessId, { favorPoint: Number(v) })} />
          </Grid.Col>
          <Grid.Col span={4}>
            <NumberInput label="模组等级" value={chess.status.equipLevel} min={0} max={3} onChange={v => patchChessStatus(chessId, { equipLevel: Number(v) })} />
          </Grid.Col>
          <Grid.Col span={4}>
            <NumberInput label="升级所需数量" value={chess.upgradeNum} min={0} onChange={v => patchChess(chessId, { upgradeNum: Number(v) })} />
          </Grid.Col>
        </Grid>
      ...
      <Grid gutter="md">
        <Grid.Col span={{ base: 12, md: 4 }}>
          <Stack gap="xs">
            ...
            <TextInput placeholder="搜索干员名或 ID..." value={search} onChange={e => setSearch(e.target.value)} size="xs" />
            <SegmentedControl
          ...
          {editing ? (
            <Stack gap="md">
              ...
              <Grid gutter="sm">
                ...
                <Grid.Col span={4}>
                  <NumberInput label="棋子阶数" value={editing.chessLevel} min={1} max={6} onChange={v => patchShop(editing.chessId, { chessLevel: Number(v) })} />
                </Grid.Col>
                <Grid.Col span={4}>
                  <NumberInput label="商店排序" value={editing.shopLevelSortId} onChange={v => patchShop(editing.chessId, { shopLevelSortId: Number(v) })} />
                </Grid.Col>
                <Grid.Col span={4}>
                  <Select
                    label="棋子类型"
                ...
                <Grid.Col span={4}>
                  <NumberInput label="默认技能索引" value={editing.defaultSkillIndex} min={0} onChange={v => patchShop(editing.chessId, { defaultSkillIndex: Number(v) })} />
                </Grid.Col>
                <Grid.Col span={4}>
                  <NumberInput label="备用干员技能索引" value={editing.backupCharSkillIndex} min={0} onChange={v => patchShop(editing.chessId, { backupCharSkillIndex: Number(v) })} />
                </Grid.Col>
                <Grid.Col span={4}>
                  <NumberInput label="备用干员潜能" value={editing.backupCharPotRank} min={0} onChange={v => patchShop(editing.chessId, { backupCharPotRank: Number(v) })} />
                </Grid.Col>
                <Grid.Col span={4}>
                  <Switch label="隐藏（不在商店显示）" checked={editing.isHidden}
