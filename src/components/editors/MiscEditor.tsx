import {
  Stack, Group, Text, Badge, Grid, NumberInput,
  ActionIcon, Title, TextInput, Divider, Switch,
  Button, Modal, Table, Tabs, Select, Textarea, MultiSelect,
} from '@mantine/core'
  ...
  return (
    <>
      ...
      <Modal opened={addOpened} onClose={closeAdd} title="新增特殊敌人" size="sm">
        <Stack gap="md">
          <TextInput label="specialEnemyKey" value={newKey} onChange={e => setNewKey(e.target.value)} onKeyDown={e => e.key === 'Enter' && add()} />
  ...
  return (
    <>
        ...
        <Grid.Col span={{ base: 12, md: 8 }}>
          {editing ? (
            <Stack gap="md">
              ...
              <Grid gutter="sm">
                <Grid.Col span={4}>
                  <NumberInput label="总 HP" value={editing.totalHp} min={0}
                    onChange={v => patch(editing.bandId, { totalHp: Number(v) })} />
                </Grid.Col>
                <Grid.Col span={4}>
                  <NumberInput label="胜利场数" value={editing.victorCount} min={0}
                    onChange={v => patch(editing.bandId, { victorCount: Number(v) })} />
                </Grid.Col>
                <Grid.Col span={4}>
                  <NumberInput label="奖励倍率" value={editing.bandRewardModulus} step={0.1} decimalScale={2}
                    onChange={v => patch(editing.bandId, { bandRewardModulus: Number(v) })} />
                </Grid.Col>
                <Grid.Col span={4}>
                  <NumberInput label="排序 ID" value={editing.sortId}
                    onChange={v => patch(editing.bandId, { sortId: Number(v) })} />
                </Grid.Col>
                {editing.updateTime !== undefined && (
                  <Grid.Col span={4}>
                    <NumberInput label="更新时间戳" value={editing.updateTime}
                      onChange={v => patch(editing.bandId, { updateTime: Number(v) })} />
                  </Grid.Col>
                )}
                <Grid.Col span={12}>
                  <Textarea label="策略描述" value={editing.bandDesc} autosize minRows={2}
                    onChange={e => patch(editing.bandId, { bandDesc: e.target.value })} />
                </Grid.Col>
                <Grid.Col span={6}>
                  <Select label="关联效果" value={editing.effectId || ''} data={effectOptions} searchable
                    onChange={v => patch(editing.bandId, { effectId: v || '' })} />
                </Grid.Col>
                <Grid.Col span={6}>
                  <MultiSelect label="适用模式类型" value={editing.modeTypeList} data={modeTypeOptions}
      ...
      <Modal opened={addOpened} onClose={closeAdd} title="新增策略组" size="sm">
        <Stack gap="md">
          <TextInput label="bandId" value={newId} onChange={e => setNewId(e.target.value)} onKeyDown={e => e.key === 'Enter' && add()} />
  ...
  return (
    <>
        ...
        <Grid.Col span={{ base: 12, md: 8 }}>
          {editing ? (
            <Stack gap="md">
              ...
              <Grid gutter="sm">
                <Grid.Col span={4}>
                  <NumberInput label="权重" value={editing.weight} min={0}
                    onChange={v => patch(editing.stageId, { weight: Number(v) })} />
                </Grid.Col>
                <Grid.Col span={12}>
                  <MultiSelect label="适用模式" value={editing.mode} data={modeOptions} searchable
      ...
      <Modal opened={addOpened} onClose={closeAdd} title="新增关卡" size="sm">
        <Stack gap="md">
          <TextInput label="stageId" value={newId} onChange={e => setNewId(e.target.value)} onKeyDown={e => e.key === 'Enter' && add()} />
          <Group justify="flex-end"><Button variant="subtle" onClick={closeAdd}>取消</Button><Button onClick={add} disabled={!newId.trim()}>创建</Button></Group>
        </Stack>
      </Modal>
      <Modal opened={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="确认删除" size="sm">
                ...
                <Table.Tbody>
                  {entries.map((entry, i) => (
                    <Table.Tr key={i}>
                      ...
                      <Table.Td>
                        <TextInput size="xs" value={entry.levelId} w={180}
                          onChange={e => {
                            updateSeason(activeSeasonId!, d => {
                      ...
                      <Table.Td>
                        <TextInput size="xs" value={entry.bossId ?? ''} placeholder="null" w={140}
                          onChange={e => {
                            updateSeason(activeSeasonId!, d => {
                              const arr = [...d.battleDataDict[mode][diff]]
                      ...
                      <Table.Td>
                        <Switch checked={entry.isSpPrepare} onChange={ev => {
                          updateSeason(activeSeasonId!, d => {
                            const arr = [...d.battleDataDict[mode][diff]]
      ...
      <Table fz="sm" withTableBorder>
        ...
        <Table.Tbody>
          {trainingNpcList.map((npc, i) => (
            <Table.Tr key={i}>
              <Table.Td>{i + 1}</Table.Td>
              <Table.Td><TextInput size="xs" value={npc.npcId} w={160} onChange={e => patch(i, { npcId: e.target.value })} /></Table.Td>
              <Table.Td>
                <Stack gap={2}>
                  <TextInput size="xs" value={npc.charId} w={160} onChange={e => patch(i, { charId: e.target.value })} />
                  <Text size="9px" c="dimmed">{getCharName(npc.charId)}</Text>
                </Stack>
              </Table.Td>
              <Table.Td><TextInput size="xs" value={npc.nameCardSkinId} w={160} onChange={e => patch(i, { nameCardSkinId: e.target.value })} /></Table.Td>
              <Table.Td><NumberInput size="xs" value={npc.medalCount} min={0} w={70} onChange={v => patch(i, { medalCount: Number(v) })} /></Table.Td>
              <Table.Td><Select size="xs" value={npc.bandId} data={bandOptions} searchable w={160} onChange={v => patch(i, { bandId: v || '' })} /></Table.Td>
      ...
      <Table fz="xs" withTableBorder>
        ...
        <Table.Tbody>
          {milestoneList.map((m, i) => (
            <Table.Tr key={i}>
              <Table.Td><TextInput size="xs" value={m.milestoneId} w={160} onChange={e => patch(i, { milestoneId: e.target.value })} /></Table.Td>
              <Table.Td><NumberInput size="xs" value={m.milestoneLvl} w={70} onChange={v => patch(i, { milestoneLvl: Number(v) })} /></Table.Td>
              <Table.Td><NumberInput size="xs" value={m.tokenNum} w={80} onChange={v => patch(i, { tokenNum: Number(v) })} /></Table.Td>
              <Table.Td><Select size="xs" value={m.rewardItem.type} data={itemTypeOptions} w={110} onChange={v => patchItem(i, { type: v as ItemType })} /></Table.Td>
              <Table.Td><TextInput size="xs" value={m.rewardItem.id} w={140} onChange={e => patchItem(i, { id: e.target.value })} /></Table.Td>
              <Table.Td><NumberInput size="xs" value={m.rewardItem.count} w={70} min={1} onChange={v => patchItem(i, { count: Number(v) })} /></Table.Td>
              <Table.Td><NumberInput size="xs" value={m.availableTime} w={100} onChange={v => patch(i, { availableTime: Number(v) })} /></Table.Td>
              <Table.Td>
                <ActionIcon size="xs" variant="subtle" color="red"
          ...
          {keys.map(k => {
            ...
            return (
              <Table.Tr key={k}>
                ...
                <Table.Td>
                  <TextInput size="xs" value={c.id} w={160}
                ...
                <Table.Td>
                  <TextInput size="xs" value={c.picId} w={160}
                ...
                <Table.Td>
                  <TextInput size="xs" value={c.txt} w={200}
...
function ConstDataEditor({ store }: Props) {
  ...
  return (
    <Stack gap="md">
      ...
      <Grid gutter="sm">
        <Grid.Col span={3}><NumberInput label="商店刷新价格" value={constData.shopRefreshPrice} onChange={v => patch({ shopRefreshPrice: Number(v) })} /></Grid.Col>
        <Grid.Col span={3}><NumberInput label="整备区最大棋子数" value={constData.maxDeckChessCnt} onChange={v => patch({ maxDeckChessCnt: Number(v) })} /></Grid.Col>
        <Grid.Col span={3}><NumberInput label="战场最大棋子数" value={constData.maxBattleChessCnt} onChange={v => patch({ maxBattleChessCnt: Number(v) })} /></Grid.Col>
        <Grid.Col span={3}><NumberInput label="仓库上限" value={constData.storeCntMax} onChange={v => patch({ storeCntMax: Number(v) })} /></Grid.Col>
        <Grid.Col span={3}><NumberInput label="借用次数" value={constData.borrowCount} onChange={v => patch({ borrowCount: Number(v) })} /></Grid.Col>
        <Grid.Col span={3}><NumberInput label="HP 扣除上限" value={constData.costPlayerHpLimit} onChange={v => patch({ costPlayerHpLimit: Number(v) })} /></Grid.Col>
        <Grid.Col span={3}><NumberInput label="日常任务参数" value={constData.dailyMissionParam} onChange={v => patch({ dailyMissionParam: Number(v) })} /></Grid.Col>
        <Grid.Col span={6}><TextInput label="fallback 盟约 ID" value={constData.fallbackBondId} onChange={e => patch({ fallbackBondId: e.target.value })} /></Grid.Col>
        <Grid.Col span={6}><TextInput label="里程碑 ID (milestoneId)" value={constData.milestoneId} onChange={e => patch({ milestoneId: e.target.value })} /></Grid.Col>
        <Grid.Col span={6}><TextInput label="里程碑追踪 ID" value={constData.milestoneTrackId} onChange={e => patch({ milestoneTrackId: e.target.value })} /></Grid.Col>
        <Grid.Col span={6}><TextInput label="日常任务名称" value={constData.dailyMissionName} onChange={e => patch({ dailyMissionName: e.target.value })} /></Grid.Col>
        <Grid.Col span={12}><TextInput label="日常任务规则" value={constData.dailyMissionRule} onChange={e => patch({ dailyMissionRule: e.target.value })} /></Grid.Col>
        <Grid.Col span={6}><TextInput label="训练关卡盟约 ID (trstageBandId)" value={constData.trstageBandId} onChange={e => patch({ trstageBandId: e.target.value })} /></Grid.Col>
        <Grid.Col span={6}><TextInput label="训练关卡 BOSS ID (trstageBossId)" value={constData.trstageBossId} onChange={e => patch({ trstageBossId: e.target.value })} /></Grid.Col>
        <Grid.Col span={6}><TextInput label="训练关卡 ID (trStageId)" value={constData.trStageId} onChange={e => patch({ trStageId: e.target.value })} /></Grid.Col>
        <Grid.Col span={6}><TextInput label="训练模式 ID (trainingModeId)" value={constData.trainingModeId} onChange={e => patch({ trainingModeId: e.target.value })} /></Grid.Col>
        <Grid.Col span={6}><TextInput label="单人逃脱战斗模板 Map" value={constData.escapedBattleTemplateMapSinglePlayer} onChange={e => patch({ escapedBattleTemplateMapSinglePlayer: e.target.value })} /></Grid.Col>
        <Grid.Col span={6}><TextInput label="多人逃脱战斗模板 Map" value={constData.escapedBattleTemplateMapMultiPlayer} onChange={e => patch({ escapedBattleTemplateMapMultiPlayer: e.target.value })} /></Grid.Col>
        <Grid.Col span={6}><TextInput label="WebBusType" value={constData.webBusType} onChange={e => patch({ webBusType: e.target.value })} /></Grid.Col>
      </Grid>
      ...
      <KeyListField label="" keys={constData.trBondIds} onChange={v => patch({ trBondIds: v })} />
      <Divider label="训练禁用盟约 IDs (trBannedBondIds)" labelPosition="left" />
      <KeyListField label="" keys={constData.trBannedBondIds} onChange={v => patch({ trBannedBondIds: v })} />
    </Stack>
  )
}
        ...
        <Table.Tbody>
          {keys.map(k => (
            <Table.Tr key={k}>
              <Table.Td><Text ff="monospace" size="xs">{k}</Text></Table.Td>
              <Table.Td>
                <TextInput size="xs" value={diyChessDict[k]} w={240}
  ...
  return (
    <>
        ...
        <Grid.Col span={{ base: 12, md: 8 }}>
          {editing ? (
            <Stack gap="md">
              ...
              <Grid gutter="sm">
                <Grid.Col span={6}>
                  <Select label="选项类型 (choiceType)" value={editing.choiceType} data={choiceTypeOptions}
                    onChange={v => patch(editing.choiceEventId, { choiceType: v as ChoiceType })} />
                </Grid.Col>
                <Grid.Col span={6}>
                  <Select label="效果类型 (effectType)" value={editing.effectType} data={effectTypeOptions}
                    onChange={v => patch(editing.choiceEventId, { effectType: v as EffectType })} />
                </Grid.Col>
                <Grid.Col span={6}>
                  <TextInput label="名称 (name)" value={editing.name}
                    onChange={e => patch(editing.choiceEventId, { name: e.target.value as any })} />
                </Grid.Col>
                <Grid.Col span={6}>
                  <TextInput label="颜色 (typeTxtColor)" value={editing.typeTxtColor}
                    onChange={e => patch(editing.choiceEventId, { typeTxtColor: e.target.value as any })} />
                </Grid.Col>
                <Grid.Col span={12}>
                  <Textarea label="描述 (desc)" value={editing.desc} autosize minRows={2}
      ...
      <Modal opened={addOpened} onClose={closeAdd} title="新增效果选项" size="sm">
        <Stack gap="md">
          <TextInput label="choiceEventId" value={newId} onChange={e => setNewId(e.target.value)} onKeyDown={e => e.key === 'Enter' && add()} />
          <Group justify="flex-end"><Button variant="subtle" onClick={closeAdd}>取消</Button><Button onClick={add} disabled={!newId.trim()}>创建</Button></Group>
        </Stack>
      </Modal>
      <Modal opened={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="确认删除" size="sm">
...
// ─────────────────────────────────────────────────────────────────────────────
function KeyListField({ label, keys, onChange }: { label: string; keys: string[]; onChange: (v: string[]) => void }) {
  return (
    <Stack gap="xs">
      {label && <Text size="sm" fw={500}>{label}</Text>}
      {keys.map((k, i) => (
        <Group key={i} gap="xs">
          <TextInput size="xs" value={k} style={{ flex: 1 }}
