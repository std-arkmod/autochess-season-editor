...
export function BondsEditor({ store }: Props) {
  ...
  useEffect(() => { updatePresence('bonds', editingId); return () => updatePresence('bonds', null) }, [editingId])
  ...
  useEffect(() => {
    if (
      focusId &&
      activeSeason?.data.bondInfoDict &&
      focusId in activeSeason.data.bondInfoDict
    ) {
  ...
  const filtered = search
    ? bondList.filter(
        (b) => b.name.includes(search) || b.bondId.includes(search),
      )
    : bondList;
  ...
  const editingEffect = editing?.effectId
    ? effectInfoDataDict[editing.effectId]
    : null;
                    ...
                    <Group justify="space-between" wrap="nowrap">
                      <div style={{ minWidth: 0 }}>
                        <Group gap="xs">
                          ...
                          <PresenceIndicator itemId={bond.bondId} />
            ...
            <Stack gap="md">
              ...
              <Grid gutter="sm">
                <Grid.Col span={6}>
                  <CTextInput
                    ...
                    value={editing.name}
                    onChange={(e) =>
                      patchBond(editing.bondId, { name: e.target.value })
                    }
                  />
                </Grid.Col>
                <Grid.Col span={3}>
                  <CNumberInput
                    ...
                    onChange={(v) =>
                      patchBond(editing.bondId, { weight: Number(v) })
                    }
                  />
                </Grid.Col>
                <Grid.Col span={3}>
                  <CNumberInput
                    ...
                    min={1}
                    onChange={(v) =>
                      patchBond(editing.bondId, { activeCount: Number(v) })
                    }
                  />
                </Grid.Col>
                <Grid.Col span={12}>
                  <CTextarea
                    ...
                    minRows={3}
                    onChange={(e) =>
                      patchBond(editing.bondId, { desc: e.target.value })
                    }
                  />
                </Grid.Col>
                <Grid.Col span={6}>
                  <CSelect
                    ...
                    onChange={(v) =>
                      patchBond(editing.bondId, {
                        activeCondition: v as ActiveCondition,
                      })
                    }
                  />
                </Grid.Col>
              ...
              <Group gap="xs" align="flex-end">
                <Box style={{ flex: 1 }}>
                  <CSelect
                    ...
                    searchable
                    onChange={(v) =>
                      patchBond(editing.bondId, { effectId: v ?? "" })
                    }
                  />
                </Box>
                {editing.effectId && (
                ...
                <Card withBorder padding="sm" bg="dark.7">
                  <Group gap="xs" mb={4}>
                    ...
                    <Badge size="xs" color="gray">
                      持续{" "}
                      {editingEffect.continuedRound === -1
                        ? "永久"
                        : `${editingEffect.continuedRound}回合`}
                    </Badge>
                  </Group>
                  <RichTextPreview
                    text={editingEffect.effectDesc}
                    maxLen={120}
                  />
                </Card>
              )}
              ...
              <Group gap="xs">
                ...
                <ActionIcon
                  ...
                  onClick={() =>
                    patchBond(editing.bondId, {
                      activeParamList: [...editing.activeParamList, ""],
                    })
                  }
                >
                  <IconPlus size={14} />
                ...
                {editing.activeParamList.length > 0 && (
                  <ActionIcon
                    ...
                    onClick={() =>
                      patchBond(editing.bondId, {
                        activeParamList: editing.activeParamList.slice(0, -1),
                      })
                    }
                  >
                    <IconTrash size={14} />
      ...
      >
        <Stack gap="md">
          ...
          <Group justify="flex-end">
            ...
            <Button
              color="red"
              onClick={() => deleteConfirm && deleteBond(deleteConfirm)}
            >
              删除
            </Button>
          </Group>
