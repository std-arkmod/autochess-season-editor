...
import { CTextInput, CNumberInput, CTextarea, CSelect, CMultiSelect, CColorInput, CollabEditingProvider } from '../collab/CollabInputs'
...
export function ModesEditor({ store }: Props) {
  ...
  useEffect(() => { updatePresence('modes', editingId); return () => updatePresence('modes', null) }, [editingId])

  // Respond to external navigation (e.g. follow mode)
  useEffect(() => {
    if (focusId && activeSeason?.data.modeDataDict && focusId in activeSeason.data.modeDataDict) {
              ...
              <Group justify="space-between" wrap="nowrap">
                <div style={{ minWidth: 0 }}>
                  <Group gap="xs">
                    ...
                    <PresenceIndicator itemId={mode.modeId} />
            ...
            <Stack gap="xs">
              {editing.inactiveEnemyKey.map((key, i) => (
                <Group key={i} gap="xs">
                  <CTextInput
                    style={{ flex: 1 }}
                    collabField={`inactiveEnemyKey[${i}]`}
                    value={key}
                    onChange={e => {
