...
import type { TrapChessDataDict, TrapShopChessData, ItemTypeEnum } from '@autochess-editor/shared'
import { getCharName, normalizeSeasonDataForRuntime } from '@autochess-editor/shared'
...
export function TrapsEditor({ store }: Props) {
  ...
  useEffect(() => { updatePresence('traps', editingId); return () => updatePresence('traps', null) }, [editingId])

  // Respond to external navigation (e.g. follow mode)
  useEffect(() => {
    if (!focusId || !activeSeason) return
    const trapDict = activeSeason.data.trapChessDataDict
    if (trapDict && focusId in trapDict) {
                      ...
                      <Group justify="space-between" wrap="nowrap">
                        <div style={{ minWidth: 0 }}>
                          <Group gap="xs">
                            ...
                            <PresenceIndicator itemId={trap.chessId} />
