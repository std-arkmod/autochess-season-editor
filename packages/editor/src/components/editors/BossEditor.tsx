...
import type { Boss } from '@autochess-editor/shared'
...
export function BossEditor({ store }: Props) {
  ...
  useEffect(() => { updatePresence('boss', editingId); return () => updatePresence('boss', null) }, [editingId])

  // Respond to external navigation (e.g. follow mode)
  useEffect(() => {
    if (focusId && activeSeason?.data.bossInfoDict && focusId in activeSeason.data.bossInfoDict) {
                    ...
                    <Group justify="space-between" wrap="nowrap">
                      <div>
                        <Group gap="xs">
                          ...
                          <PresenceIndicator itemId={boss.bossId} />
        ...
        <Grid.Col span={{ base: 12, md: 8 }}>
          {editing ? (
            ...
            <Stack gap="md">
              ...
              <Grid gutter="sm">
                <Grid.Col span={6}>
                  <CNumberInput label="基础血量" value={editing.bloodPoint} min={0} onChange={v => patchBoss(editing.bossId, { bloodPoint: Number(v) })} />
                </Grid.Col>
                <Grid.Col span={6}>
                  <CNumberInput label="普通难度血量" value={editing.bloodPointNormal} min={0} onChange={v => patchBoss(editing.bossId, { bloodPointNormal: Number(v) })} />
                </Grid.Col>
                <Grid.Col span={6}>
                  <CNumberInput label="困难难度血量" value={editing.bloodPointHard} min={0} onChange={v => patchBoss(editing.bossId, { bloodPointHard: Number(v) })} />
                </Grid.Col>
                <Grid.Col span={6}>
                  <CNumberInput label="深渊难度血量" value={editing.bloodPointAbyss} min={0} onChange={v => patchBoss(editing.bossId, { bloodPointAbyss: Number(v) })} />
                </Grid.Col>
              </Grid>
