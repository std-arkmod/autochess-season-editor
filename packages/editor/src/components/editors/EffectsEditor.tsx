...
import { effectTypeLabel } from '@autochess-editor/shared'
import { RichTextPreview } from '../shared/RichTextPreview'
import type { DataStore } from '../../store/dataStore'
import { PresenceIndicator } from '../collab/PresenceIndicator'
import { useCollab } from '../../context/CollabContext'
import type { EffectInfoDataDict, EffectType } from '@autochess-editor/shared'
                    ...
                    <Group justify="space-between" wrap="nowrap">
                      <div style={{ minWidth: 0 }}>
                        <Group gap="xs">
                          ...
                          <PresenceIndicator itemId={effect.effectId} />
              ...
              {editingBuffs.map((buff, bi) => (
                <Card key={bi} withBorder padding="sm">
                  <Group justify="space-between" mb="xs">
                    <Group gap="xs">
                      <CTextInput
                        size="xs" label="Buff Key" collabField={`buff[${bi}].key`} value={buff.key} w={200}
                        onChange={e => editing && patchBuff(editing.effectId, bi, { key: e.target.value })}
                      />
                      <CSelect
                        size="xs" label="计数类型" collabField={`buff[${bi}].countType`} value={buff.countType} w={120}
                  ...
                  <Table fz="xs" withTableBorder>
                    ...
                    <Table.Tbody>
                      {buff.blackboard.map((bb, i) => (
                        <Table.Tr key={i}>
                          <Table.Td>
                            <CTextInput size="xs" collabField={`buff[${bi}].bb[${i}].key`} value={bb.key} w={140}
                              onChange={e => editing && patchBlackboard(editing.effectId, bi, i, { key: e.target.value })} />
                          </Table.Td>
                          <Table.Td>
                            <CNumberInput size="xs" collabField={`buff[${bi}].bb[${i}].value`} value={bb.value} step={0.01} decimalScale={4} w={100}
                              onChange={v => editing && patchBlackboard(editing.effectId, bi, i, { value: Number(v) })} />
                          </Table.Td>
                          <Table.Td>
                            <CTextInput size="xs" collabField={`buff[${bi}].bb[${i}].valueStr`} value={bb.valueStr ?? ''} placeholder="null" w={100}
